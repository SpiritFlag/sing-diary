...
> **구현 현황 (expand-playlist-import 사이클)**: **본인 곡 분기만 구현되어 있다.**
> 타인 곡(공개 세션) 분기와 사본 복사는 미구현이다 — 공개 세션을 열람할 화면 자체가 아직
> 없어서(세션 목록·상세는 owner 스코프 전용) 사본 정책을 실행할 진입점이 없다. 위 사본 정책은
> 그 진입점이 생길 때 적용할 설계로 남긴다.
...
> **구현 현황** (expand-fill-queue, 2026-08-23)
> 위 두 쿼리는 `SongQuery.fillQueue`가 `NOT EXISTS`로 이행한다(의미 동일). 정렬은 `created_at ASC` —
> `updated_at`은 저장이 touch하므로 정렬 키로 쓰면 한 건 저장할 때마다 카드가 튄다.
> 화면은 `/songs/fill` 한 화면 세 탭(TJ 번호 · KY 번호 · 제목·아티스트)이고, 스냅샷은 입장 시 1회
> 로드한 뒤 클라이언트가 소모한다 — 서버 재조회는 화면 안의 새로고침 버튼뿐이다.
> **미구현 — 외부 검색 링크.** 어느 사이트를 붙일지가 정해지지 않아 이번 사이클에서 뺐다.
> **저장은 낙관적 이동이다.** 응답을 기다리지 않고 다음 건으로 넘어가며(쓰기가 3~4초인 상수 때문),
> 실패한 건은 입력값을 실은 채 그 탭 큐 끝으로 되돌아온다. 그 대가로 이탈 경고와 전송 정산 표시가 붙는다.
...
| D8 | 곡 관리 표의 내부 검색은 **클라이언트 substring 필터**로 확정 (통합검색 API를 부르지 않는다) | 표는 이미 owner의 전체 목록을 로드해두므로 클라이언트 필터가 왕복 0회로 같은 결과를 낸다. 1인 수백 곡 규모에서 우위가 명확하다. Plan→Design 사이에 무기록으로 바뀌어 있던 것을 여기서 결정으로 승격한다(expand-fill-queue Design §2.3 D-H). **재검토 조건: 곡 목록에 페이지네이션이 필요해지는 시점** |
